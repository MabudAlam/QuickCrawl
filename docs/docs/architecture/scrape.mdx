...
### Browser WS URL resolution

The browser's `webSocketDebuggerUrl` is resolved once at server startup via `GET /json/version` (`core.GetCDPURL`) and cached on the `Renderer`. Every `/v1/scrape` request reuses the same persistent `RemoteAllocator` — no per-request discovery, no per-request HTTP call.

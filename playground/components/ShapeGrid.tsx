...
const ShapeGrid = ({
  ...
  const colorsRef = useRef({ borderColor: "#2F293A", hoverFillColor: "#FB923C" });

  const getThemeColors = () => {
    const isDark = document.documentElement.classList.contains("dark") ||
      window.matchMedia("(prefers-color-scheme: dark)").matches;
    if (isDark) {
      return { borderColor: "#2F293A", hoverFillColor: "#F97316" };
    }
    return { borderColor: "#2F293A", hoverFillColor: "#FB923C" };
  };
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            if (alpha) {
              ...
              ctx.fillStyle = colorsRef.current.hoverFillColor;
              ctx.fill();
              ctx.globalAlpha = 1;
            }

            drawHex(cx, cy, squareSize);
            ctx.strokeStyle = colorsRef.current.borderColor;
            ctx.stroke();
          }
        }
      } else if (isTri) {

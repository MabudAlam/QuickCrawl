...
func (f *HTTPFetcher) Fetch(rawURL string, headers map[string]string, waitForMs *int64) (*types.FetchResult, *types.QuickCrawlError) {
	...
	for attempt := 0; attempt <= utils.HTTPMaxRetries; attempt++ {
		...
		if attempt > 0 {
			backoff := utils.HTTPRetryBackoff
		...
		if err != nil {
			...
			if attempt < utils.HTTPMaxRetries && isRetriableErrStr(lastErrStr) {
		...
		if attempt < utils.HTTPMaxRetries && isRetriableStatus(statusCode) {
